...
function flushGuests(cache:NodeCache){
    const now = Date.now();
    const guests:Guests = cache.get('guests');
...
export function connectNewGuest(params, res, cache:NodeCache){
    const guests:Guests = cache.get('guests'); // set inside server start
...
function connectGuest(id, res, cache:NodeCache){
...
export function getGuests(cache:NodeCache){
    const guests:Guests = cache.get('guests');
    const activeGuests = guests.guests.reduce((arr:Guest[], guest:Guest) => {
        if(guest.active){
            arr.push(guest);
        }
        return arr;
...
export function connectUser(res){
    console.log('connecting user');
    sendJson(res, {});
}

export function connect(req, res, cache:NodeCache){
...
export function disconnect(req, res, cache:NodeCache){
    const guests:Guests = cache.get('guests');
...
export function connectRoutes(app, cache:NodeCache){
...
export function renameUser(req, res, cache:NodeCache){
    const guests:Guests = cache.get('guests');
    const params = req.body;
    if(params.user){
        if(params.user.type === 'guest'){
            ...
            sendJson(res, {user: newGuests.guests[params.user.id]});
        }else{
            sendJson(res);
        }
    }else{
...
export function serverCheck(req, res, cache:NodeCache){
    const guests:Guests = cache.get('guests');
    const params = req.body;
    if(params.user){
